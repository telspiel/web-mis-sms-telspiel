...
import "./GroupCampaign.css";

function GroupCampaign({ userData, onLogout }) {
    ...
    const [shortUrlForUser, setShortUrlForUser] =  useState([]);
    const [groupContactList, setGroupContactList] = useState([]);
    const [selectedGroup, setSelectedGroup] = useState(""); // Store selected groupId
  ...
  const fetchShortUrlList = async () => {
    try {
      ...
      if (data?.data?.shortUrlList) {
        setShortUrlForUser(data.data.shortUrlList); // Update state with shortUrlList
      }
    } catch (error) {
...
//Fetch All Group Contacts List
const fetchGroupContactslist = async () => {
  try {
    const response = await fetch(Endpoints.get("getAllGroupList"), {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: userData.authJwtToken,
    ...
    if (data?.data?.groupList) {
      ...
      setGroupContactList(data.data.groupList);
    } else {
...
const fetchAlltemplateList = async () => {
  try {
    const response = await fetch(Endpoints.get('getAllTemplateData'), {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: userData.authJwtToken,
    ...
    if (data?.data?.templateList) {
      setAllTemplateList(data.data.shortUrlList); // Update state with shortUrlList
    }
  } catch (error) {
    console.error('Error fetching Short URL List:', error);
  }
...
useEffect(() => {
  ...
  //   if (selectedTemplate) {
  //     setIsFromTemplate(true); // Make inputs read-only when a template is selected
...
//   const charCount = text.length; // Count everything as-is
...
const handleScheduleMessageChange = (event) => {
  setScheduleMessage(event.target.value);
...
//       const urlData = shortUrlForUser.find(item => item.name === name);
//       if (urlData) {
...
//     const url = shortUrlForUser.find(item => item.name === latestAdded);
//     if (url) {
...
const handleShortUrlChange = (selectedOptions) => {
  ...
  // Track the removed index to insert the next one at the same position
  if (removedNames.length > 0) {
  ...
  if (addedNames.length > 0) {
    ...
    if (url) {
      ...
      if (parts.length >= 3) {
        ...
        setLastRemovedIndex(null); // reset after use
      }
    }
  } else {
    // Only removal case, or deselecting all
    setSelectedShortUrls(newSelectedNames);
  }
};
...
const handleGroupChange = (event) => {
  ...
  const selectedGroupObj = groupContactList.find(
    (group) => String(group.groupId) === selectedGroupId
  );

  if (selectedGroupObj) {
    setGroupName(selectedGroupObj.groupName); // Set group name
  }
};
...
const handleScheduleHourChange = (event) => {
  setScheduleHour(event.target.value);
};

const handleScheduleMinuteChange = (event) => {
  setScheduleMinute(event.target.value);
};


//==========================TO SAVE GROUP CAMPAIGN DETAILS==============================
const saveGroupCampaign = async () => {
  // const payload = {
  ...
  //   isScheduledSMS: String(scheduleMessage),
  //   scheduleDate: scheduleMessage === "yes" ? scheduleDate : "",
  //   scheduleHour: scheduleMessage === "yes" ? scheduleHour : "",
  //   scheduleMinute: scheduleMessage === "yes" ? scheduleMinute : "",
  ...
  //   }),
  //   shortUrlName: isShortUrlSelected === "Y" ? [selectedShortUrl] : [],
  // };
  ...
  try {
    // API call
    const response = await fetch(Endpoints.get("sendGroupCampaign"), {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: userData.authJwtToken,
  ...
  return (
    <div className="dashboard-container">
        ...
        <div className="dashboard-content">
            ...
            <form className="group-campaign-form" onSubmit={saveGroupCampaign}>
            <div className="quick-input-field">
              ...
              <div className="quick-input-field">
                ...
                <select required value={selectedGroup} onChange={handleGroupChange}>
            ...
            <div className="upload-input-field">
              ...
              <div className="groupWrapHourTime">
              <select
                className="hourAndMinute"
                value={scheduleHour}
                onChange={(e) => {
              ...
              {isShortUrlSelected === "Y" && (
                ...
                options={shortUrlForUser.map((url) => ({
                  label: url.name,
                  value: url.name,
                }))}
            ...
            <div className="quick-input-field">
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  setIsFromTemplate(false); // Allow manual editing again
                }}
                // required
